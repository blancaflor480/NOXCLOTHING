...
<html lang="en" >
	<head>
	  <meta charset="UTF-8">
		<title> Admin Sign In | NOX CLOTHING</title>
	...
	<?php 
			...
			if (isset($_POST['login'])) {
			  ...
			  if(mysqli_num_rows($result) === 1){
				...
				$update_login_time_query = "UPDATE admin SET logintime = '$current_datetime' WHERE uname = '$uname'";
				mysqli_query($conn, $update_login_time_query);
...
if (strlen($password) < 8 ||  !preg_match('/[\'^£$%&*()}{@#~?><>,|=_+¬-]/', $password)) {
	$errors['password'] = 'Password should be at least 8 characters long and contain a special character.';
}
...
if (mysqli_num_rows($check_uname_result) > 0) {
	$errors['uname'] = "Username already existing";
}

if(empty($errors)){
	$query =  "INSERT into user (uname,fullname,email,password,bday) VALUES ('$uname','$fullname','$email','".md5($password)."','$bday')";
	...
	if($result){
		...
		echo "<script>
		alertify.set('notifier','position', 'top-center');
		alertify.error('Failed. Try agin');
...
?>
            <h1>CREATE ACCOUNT.</h1>
				...
				<?php if (isset($errors['email'])) echo '<span style ="color: red; font-size: 0.6rem; text-align: right;">' . $errors['email'] . '</span>'; ?>
				</span>
				...
				<?php if (isset($errors['uname'])) echo '<span style ="color: red; font-size: 0.6rem; text-align: left;">' . $errors['uname'] . '</span>'; ?>
				</span>

				<input type="password" name="password" id="password" placeholder="Password" required oninput="validatePassword()" />
				<?php if (isset($errors['password'])) echo '<span style ="color: red; font-size: 0.6rem; text-align: left;">' . $errors['password'] . '</span>'; ?>
