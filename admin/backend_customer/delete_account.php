...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    if (isset($_POST['productID'])) {
        ...
        if ($stmt->execute()) {
            ...
            header("Location: ../customer.php?success=Product%20successfully%20deleted");
            exit();
        } else {
            // May error sa pag-delete ng produkto
            header("Location: ../customer.php?error=Failed%20to%20delete%20product");
            exit();
        }
    } else {
        // Walang productID na natanggap
        header("Location: ../customer.php?error=Product%20ID%20not%20provided");
        exit();
    }
} else {
    // Hindi POST request
